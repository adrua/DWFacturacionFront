...
@Component({
  selector: 'cnt-facturamovimientos-table',
  templateUrl: './cnt.facturamovimientos.table.html',
  // styleUrls: ['./cnt.facturamovimientos.table.css'],
  providers: [CntFacturaMovimientosService]
})
export class CntFacturaMovimientosTable implements AfterViewInit {
    rows: CntFacturaMovimientosModel[] = [];
    originals: CntFacturaMovimientosModel[] = [];
    selectedRow: CntFacturaMovimientosModel;
    selectedIndex: number = 0;
    originalRow: CntFacturaMovimientosModel;

    public displayedColumns: string[] = ['invProductos/productoDescripcion', 'facturaMovimientoCantidad', 'facturaMovimientoValorUnidad', 'facturaMovimientoTotal'];
    ...
    @Input() masterRow: CntFacturasModel;
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.cntFacturaMovimientosService.getList(this.masterRow, _filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new CntFacturaMovimientosModel();
      this.originalRow = new CntFacturaMovimientosModel();
      this.selectedIndex = 0;
      this.selectedRow.FacturaId = this.masterRow.FacturaId;
      this.selectedRow.FacturaSerie = this.masterRow.FacturaSerie;
    ...
    onSelect(e: Event, row: CntFacturaMovimientosModel, index: number) {
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = CntFacturaMovimientosModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: CntFacturaMovimientosModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = CntFacturaMovimientosModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.cntFacturaMovimientosService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(CntFacturaMovimientosModel.cloneExcel(data.value), 'FacturaMovimientos');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.cntFacturaMovimientosService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new CntFacturaMovimientosModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new CntFacturaMovimientosModel().fromClipboard(line);
            });

            this.cntFacturaMovimientosService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for CntFacturaMovimientos');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(CntFacturaMovimientosDialog, {
          data: {
          ...
          if (result) {
            ...
            switch (this.selectedRow._estado )
            {
              case 'N':
                ...
                let data = {
                  FacturaValor: this.rows.reduce((total, row) => total + row.FacturaMovimientoTotal, 0.0)
                };
