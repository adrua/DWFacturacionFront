...
@Component({
  selector: 'cnt-codigosciiu-table',
  templateUrl: './cnt.codigosciiu.table.html',
  // styleUrls: ['./cnt.codigosciiu.table.css'],
  providers: [CntCodigosCiiuService]
})
export class CntCodigosCiiuTable implements AfterViewInit {
    rows: CntCodigosCiiuModel[] = [];
    originals: CntCodigosCiiuModel[] = [];
    selectedRow: CntCodigosCiiuModel;
    selectedIndex: number = 0;
    originalRow: CntCodigosCiiuModel;

    public displayedColumns: string[] = ['codigoCiiuId', 'codigoCiiuDescripcion', 'codigoCiiuclase', 'codigoCiiugrupo', 'codigoCiiudivision', 'codigoCiiuBloqueo'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.cntCodigosCiiuService.getList(_filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new CntCodigosCiiuModel();
      this.originalRow = new CntCodigosCiiuModel();
    ...
    onSelect(e: Event, row: CntCodigosCiiuModel, index: number) {
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = CntCodigosCiiuModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: CntCodigosCiiuModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = CntCodigosCiiuModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.cntCodigosCiiuService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(CntCodigosCiiuModel.cloneExcel(data.value), 'CodigosCiiu');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.cntCodigosCiiuService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new CntCodigosCiiuModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new CntCodigosCiiuModel().fromClipboard(line);
            });

            this.cntCodigosCiiuService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for CntCodigosCiiu');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(CntCodigosCiiuDialog, {
          data: {
