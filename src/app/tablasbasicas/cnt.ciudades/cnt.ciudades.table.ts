...
@Component({
  selector: 'cnt-ciudades-table',
  templateUrl: './cnt.ciudades.table.html',
  // styleUrls: ['./cnt.ciudades.table.css'],
  providers: [CntCiudadesService]
})
export class CntCiudadesTable implements AfterViewInit {
    rows: CntCiudadesModel[] = [];
    originals: CntCiudadesModel[] = [];
    selectedRow: CntCiudadesModel;
    selectedIndex: number = 0;
    originalRow: CntCiudadesModel;

    public displayedColumns: string[] = ['ciudadDepartamentoId', 'ciudadid', 'ciudadCodigoPoblado', 'ciudadNombreDepartamento', 'ciudadNombreCiudad', 'ciudadNombrePoblado', 'ciudadTipoMunicipio'];
        ...
        merge(this.sort.sortChange, this.paginator.page)
          .pipe(
            ...
            switchMap(() => {
              ...
              return this.cntCiudadesService.getList(_filter, this.paginator, this.sort);
            }),
            map(data => {
    ...
    onAdd(): void {
      this.selectedRow = new CntCiudadesModel();
      this.originalRow = new CntCiudadesModel();
    ...
    onSelect(e: Event, row: CntCiudadesModel, index: number) {
        this.selectedRow = row;
        this.selectedIndex = index;
        this.originalRow = CntCiudadesModel.clone(row);
    }

    onSelectAndEdit(e: Event, row: CntCiudadesModel, index: number) {
        this.selectedRow = row;
        this.selectedRow._estado = 'O';
        this.selectedIndex = index;
        this.originalRow = CntCiudadesModel.clone(row);
    ...
    onExportExcel(e: Event) {
        this.isLoadingResults = true;
        this.cntCiudadesService.getAll().subscribe(data => {
            if (data.value.length == 0) {
                ...
                DescargaExcelModule.generarArchivoExcel(CntCiudadesModel.cloneExcel(data.value), 'Ciudades');
                this.isLoadingResults = false;
            }
        });
    }
    ...
    onCopyRows(e: Event) {
        ...
        this.cntCiudadesService.getAll().subscribe((data: any) => {
            data.forEach((row) => result += new CntCiudadesModel(row).toClipboard() + '\n');
    ...
    onPasteRows(e: Event) {
        (navigator as any).clipboard.readText().then((text: string) => {
            let rows = text.split("\n").filter((line) => line.length > 0).map((line) => {
                return new CntCiudadesModel().fromClipboard(line);
            });

            this.cntCiudadesService.saveRows(rows).subscribe((data: any) => {
                console.log('Saved rows for CntCiudades');
                this.paginator.page.emit();
            });
        });
    }
    ...
    openDialog(): void {
        const dialogRef = this.dialog.open(CntCiudadesDialog, {
          data: {
